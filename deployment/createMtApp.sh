...
az keyvault secret set --name mt-aad-password --vault-name $KEYVAULT_NAME --description "used by middle tier to access all Azure resources" --value $SERVICE_PRINCIPAL_PASSWORD
...
az keyvault set-policy --secret-permissions get --resource-group $RESOURCE_GROUP --name $MT_KEYVAULT_NAME --spn http://$SERVICE_PRINCIPAL_NAME
